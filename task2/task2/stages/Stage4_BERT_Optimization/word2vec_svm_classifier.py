"""
word2vec_svm_classifier.py (增强版)
===========================
Word2Vec + SVM分类器实现 - 添加了模型保存/加载功能
使用Word2Vec词嵌入和支持向量机
"""
...
class Word2VecSVMClassifier:
    ...
    def __init__(self, vector_size=100, window=5, min_count=2, epochs=10, 
                 use_linear_svm=True, add_features=True, model_path='models/word2vec_svm_model'):
        """
        初始化Word2Vec+SVM分类器
    ...
    def save_model(self):
        ...
        # 保存Word2Vec模型
        w2v_path = f"{self.model_path}_w2v.model"
        self.w2v_model.save(w2v_path)
        
        # 保存SVM和配置
        svm_path = f"{self.model_path}_svm.pkl"
        model_data = {
            ...
            'min_count': self.min_count,
            'epochs': self.epochs,
            'use_linear_svm': self.use_linear_svm,
            'add_features': self.add_features,
            'is_trained': self.is_trained
        }
        joblib.dump(model_data, svm_path)
    ...
    def load_model(self):
        ...
        if not os.path.exists(w2v_path) or not os.path.exists(svm_path):
            print(f"⚠️  模型文件不存在:")
        ...
        # 加载SVM和配置
        model_data = joblib.load(svm_path)
        ...
        self.use_linear_svm = model_data['use_linear_svm']
        self.add_features = model_data['add_features']
...
def main():
    ...
    print("="*70)
    print(" Word2Vec + SVM 分类器演示（增强版 - 支持保存/加载）")
    print("="*70)
    ...
    classifier = Word2VecSVMClassifier(
        ...
        min_count=2,
        epochs=10,
        model_path='models/word2vec_svm_model'
    )
